...
function CharacterNameText({ field }: { field: ReturnType<typeof cosplanCharacterNames>[number] }) {
  ...
  const fontSize = useMemo(() => {
    const measure = new Konva.Text({ text, fontSize: field.fontSize, fontFamily: "Arial", fontStyle: field.bold ? "bold" : "normal" });
    const fitted = Math.min(field.fontSize, Math.max(1, field.height - 4) / 1.15, field.fontSize * Math.max(1, field.width - 4) / Math.max(1, measure.width()));
    measure.destroy();
    return fitted;
  }, [text, field.fontSize, field.height, field.width, field.bold]);
  return <Text text={text} x={field.x} y={field.y} width={field.width} height={field.height} fontSize={fontSize} fontFamily="Arial" fontStyle={field.bold ? "bold" : "normal"} fill={field.fill} align={field.align} verticalAlign="middle" wrap="none" padding={2} listening={false} />;
}
...
export default function CosplanEditor({ templates }: { templates: CosplanTemplateSummary[] }) {
  ...
  function duplicateLayer(layer: CosplanLayer) {
    ...
    const copy = { ...layer, id: crypto.randomUUID(), name: layer.type === "image" ? layer.name : `${layer.name} ${t("copy")}`, x: layer.x + 24, y: layer.y + 24 };
  ...
  const nameVisibilityControl = selected?.type === "image" && slots.some((slot) => slot.id === selected.slotId && slot.nameText)
    ? <label className="flex min-h-11 items-center gap-3 rounded-lg border border-border bg-control px-3 py-2 text-sm font-semibold text-fg-muted">
        <input type="checkbox" checked={selected.showName !== false} onChange={(event) => updateLayer(selected.id, { showName: event.target.checked })} />
        {t("showCharacterName")}
      </label>
    : null;
          ...
          {composition.layers.map((layer, index) => {
            ...
            return <li key={layer.id}>
              <button type="button" onClick={() => {
                setSelectedId(layer.id);
                if (layer.type === "text" && typeof window !== "undefined" && window.matchMedia("(max-width: 1023px)").matches) setMobilePanel("text");
              }} className={`flex min-h-11 w-full items-center gap-3 rounded-lg border px-3 text-left ${selectedId === layer.id ? "border-accent bg-accent-surface" : "border-border bg-surface"}`}>
  ...
  const mobileTextTools = <div className="flex flex-col gap-4">
    ...
    {selected?.type === "text" ? <div className="grid gap-4 rounded-xl border border-border bg-surface p-3">
      ...
      <div>
        <div className="mb-1 flex items-center justify-between gap-3"><label htmlFor={`mobile-font-size-${selected.id}`} className="text-sm font-semibold text-fg-muted">{t("fontSize")}</label><span className="font-meta text-xs text-fg-subtle">{Math.round(selected.fontSize)} PX</span></div>
        <input id={`mobile-font-size-${selected.id}`} type="range" min="10" max="500" value={selected.fontSize} onChange={(event) => updateLayer(selected.id, { fontSize: Number(event.target.value) })} className="h-11 w-full accent-[var(--color-accent)]" />
      </div>
      ...
      <details open={textMore} onToggle={(event) => setTextMore(event.currentTarget.open)} className="group rounded-lg border border-border bg-raised">
        ...
        <div className="grid gap-3 border-t border-border p-3">
          ...
          <div className="grid grid-cols-2 gap-2"><Button size="compact" onClick={() => duplicateLayer(selected)}>{t("duplicate")}</Button><Button size="compact" onClick={() => reorderLayer(selected.id, 1)}>{t("bringForward")}</Button><Button size="compact" onClick={() => reorderLayer(selected.id, -1)}>{t("sendBackward")}</Button><Button size="compact" variant="danger" onClick={() => removeLayer(selected.id)}>{t("delete")}</Button></div>
        </div>
      </details>
    </div> : <p className="rounded-lg border border-dashed border-border px-3 py-4 text-sm text-fg-subtle">{t("selectTextLayer")}</p>}
  </div>;
  ...
  return <CanvasLayoutContext.Provider value={{ slots, foreground, width: composition?.width ?? 0, height: composition?.height ?? 0 }}><div inert={textSession ? true : undefined} className="relative pb-20 lg:pb-0">
    ...
    {composition && !textSession && <section aria-label={t("tools")} className="fixed bottom-[max(0.5rem,env(safe-area-inset-bottom))] left-1/2 z-40 flex w-[calc(100%-1rem)] max-w-2xl -translate-x-1/2 flex-col overflow-hidden rounded-xl border border-border-strong bg-raised/95 shadow-[0_16px_48px_rgb(0_0_0/0.24)] backdrop-blur-xl lg:hidden">
      ...
      {mobilePanel && <div id="cosplan-mobile-tool-panel" className="max-h-[46dvh] min-h-0 overflow-y-auto overscroll-contain px-4 pb-4">
        <div className="sticky top-0 z-10 -mx-4 mb-3 flex min-h-11 items-center justify-center border-b border-border bg-raised/95 px-4 backdrop-blur-xl">
          ...
          <button type="button" onClick={() => setMobilePanel(null)} aria-label={t("collapseTools")} className="absolute right-1 top-0 flex size-11 items-center justify-center rounded-lg text-lg text-fg-muted hover:bg-accent-surface focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent/45"><span aria-hidden="true">⌄</span></button>
        </div>
        {panelContent[mobilePanel]}
      </div>}
      <nav aria-label={t("tools")} className={`${mobilePanel ? "border-t border-border" : ""} grid grid-cols-5 p-1`}>{(["background", "character", "text", "layers", "export"] as const).map((panel) => {
    ...
    {textSession && composition && <InlineTextEditor layer={textSession.layer} value={textSession.value}
      ...
      {cosplanCharacterNames(slots, composition.layers).map((field) => <CharacterNameText key={`name-${field.slotId}`} field={field} />)}
    </InlineTextEditor>}
