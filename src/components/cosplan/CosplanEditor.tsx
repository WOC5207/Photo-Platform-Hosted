...
export default function CosplanEditor({ templates }: { templates: CosplanTemplateSummary[] }) {
  ...
  function addText() {
    ...
    apply({ ...composition, layers: [...composition.layers, layer] }); setSelectedId(layer.id); setMobilePanel(null);
  }
        ...
        <ul className="flex flex-col-reverse gap-2">
          {composition.layers.map((layer, index) => {
            ...
            return <li key={layer.id}>
              <button type="button" onClick={() => { setSelectedId(layer.id); setMobilePanel(null); }} className={`flex min-h-11 w-full items-center gap-3 rounded-lg border px-3 text-left ${selectedId === layer.id ? "border-accent bg-accent-surface" : "border-border bg-surface"}`}>
  ...
  const panelContent: Record<NonNullable<typeof mobilePanel>, ReactNode> = { background: backgroundTools, character: characterTools, text: textTools, layers: <div className="flex flex-col gap-6">{layerTools}<div className="border-t border-border pt-5">{inspector}</div></div>, export: exportTools };
